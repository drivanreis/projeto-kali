...
## Visão Geral
O KALI-CORE é um sistema unificado de intrusão e monitoramento de rede, projetado para operações de reconhecimento e penetração em tempo real. O sistema é composto por módulos independentes que operam em threads separadas, orquestrados pelo `main.py`.

**Nova Prioridade de Inteligência:** O alvo não é um servidor isolado, mas uma infraestrutura baseada em MPLS e BIND sob um ASN da MOB.

**Definição de Vitória:** O objetivo final não é a destruição, mas a marcação de território (Proof of Concept). A vitória é definida por três ações de baixo impacto e alto valor:
...
## Alvo Atual
**IP:** 138.122.82.214
**Portas de interesse:** 80, 443, 3333, 3389, 5900+, 7547, 8080, 6568, 7070, 53 (BIND/DNS)
